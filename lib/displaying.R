...
wordcloud.comparison <- function (arg1, arg2) {
  # Worcloud
  term1 <- paste0(arg2,"-1")
  term2 <- paste0(arg2,"-2")
  selected.data1 <- arg1[arg1$document==term1,]
  selected.data2 <- arg1[arg1$document==term2,]
...
ggplot.flow <-function(corpus, president){
  ...
  ggplot(speech.flow, aes(section, score, fill=colour)) +
